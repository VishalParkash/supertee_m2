    ...
    public function execute()
{
        ...
        // array('like' => '% '.$search_text) 
        // );
        $collection->addAttributeToSelect('name')->addAttributeToFilter('name', array('like' => '%'.$search_text.'%'));
        ...
        if(!empty($collection->getData())){
            ...
            foreach($collection->getData() as $products){
                $childproductcheck = $childproduct->getParentIdsByChild($products['entity_id']);
                if (isset($childproductcheck[0])) {
                    $data[] = $childproductcheck[0];
                } else {
                    $data[] = $products['entity_id'];
                }
            }
        }else{
            $data = array();
